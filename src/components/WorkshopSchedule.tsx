...
const WorkshopSchedule = () => {
  const schedule = [
    ...
    { day: "Day 6", title: "Mystery Day", description: "This one's a surprise. We've got something special planned that's going to be loaded with value. Just know it's worth showing up for.", activities: ["Details dropping soon", "You'll want to be there", "Expect the unexpected"], golden: true },
  ];
          ...
          {schedule.map((day, i) => (
            <div key={i} className={`group relative overflow-hidden border ${day.golden ? 'border-yellow-400/40 dark:border-yellow-500/15' : 'border-slate-200 dark:border-white/10'} bg-white dark:bg-black ${day.golden ? 'hover:border-yellow-500/60 dark:hover:border-yellow-500/25' : 'hover:border-emerald-500/40 dark:hover:border-white/20'} transition-all rounded-2xl shadow-md hover:shadow-xl`}>
              <div className="flex flex-col sm:flex-row">
                <div className={`sm:w-32 p-5 ${day.golden ? 'bg-gradient-to-br from-yellow-500 to-yellow-700 dark:from-yellow-600 dark:to-yellow-800' : 'bg-gradient-to-br from-emerald-700 to-emerald-900 dark:from-[#2d6a4f] dark:to-[#1b4332]'} text-white flex items-center justify-center font-black text-xl font-manrope shrink-0`}>
                  {day.day}
                </div>
