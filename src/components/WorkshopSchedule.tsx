    ...
    <section className="relative z-10 py-24 px-6" aria-labelledby="workshop-schedule">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="space-y-5">
          {schedule.map((day, i) => (
            <div key={i} className="group relative overflow-hidden border border-slate-200 dark:border-white/10 bg-white dark:bg-black hover:border-emerald-500/40 dark:hover:border-white/20 transition-all rounded-2xl shadow-md hover:shadow-xl">
              <div className="flex flex-col sm:flex-row">
                <div className="sm:w-32 p-5 bg-gradient-to-br from-emerald-700 to-emerald-900 dark:from-[#2d6a4f] dark:to-[#1b4332] text-white flex items-center justify-center font-black text-xl font-manrope shrink-0">
                  {day.day}
                </div>
                <div className="flex-1 p-6 md:p-8 space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-2.5">
                    {day.activities.map((a, j) => (
                      <div key={j} className="p-3 border border-slate-200/80 dark:border-zinc-800 rounded-xl text-xs font-semibold text-slate-700 dark:text-zinc-300 bg-slate-50/80 dark:bg-zinc-900/50 flex items-center gap-2">
                        <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600 dark:text-[#52b788] shrink-0" /> {a}
                      </div>
                    ))}
                  </div>
                </div>
